...
class VAEXperiment(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx, optimizer_idx=0):
        images, labels = batch
        real_img = torch.cat([images[0], images[1]], dim=0)
        self.curr_device = real_img.device

        results = self.forward(real_img, labels=labels)

        train_loss = self.model.loss_function(*results, labels=labels, M_N=self.params['kld_weight'],
    ...
    def validation_step(self, batch, batch_idx, optimizer_idx=0):
        ...
        results = self.forward(real_img, labels=labels)
        val_loss = self.model.loss_function(*results, labels=labels, M_N=1.0,  # real_img.shape[0]/ self.num_val_imgs,
                                            optimizer_idx=optimizer_idx, batch_idx=batch_idx)
    ...
    def sample_images(self):
        ...
        vutils.save_image(recons.data, os.path.join(self.logger.log_dir, "Reconstructions",
                                                    f"recons_{self.logger.name}_Epoch_{self.current_epoch}.png"),
                          normalize=True, nrow=12)

        try:
            samples = self.model.sample(144, self.curr_device, labels=test_label)
            vutils.save_image(samples.cpu().data, os.path.join(self.logger.log_dir, "Samples",
